...
function EmptyState({ icon: Icon, title, description }) {
  return (
    <div className="surface-inset rounded-2xl border border-dashed px-4 py-10 text-center">
      <Icon className="mx-auto mb-3 h-10 w-10 text-slate-400" aria-hidden />
      <p className="text-heading text-sm font-bold">{title}</p>
      <p className="text-muted mx-auto mt-2 max-w-sm text-xs leading-relaxed">{description}</p>
    </div>
  );
}
  ...
  return (
    <div className="mt-3 border-t border-slate-200 pt-3 dark:border-slate-600">
      <button
        ...
        className="text-muted flex w-full items-center justify-between text-xs font-semibold uppercase tracking-wide"
        aria-expanded={open}
      >
        <span>
          Instructions ({stepCount} step{stepCount === 1 ? '' : 's'})
        </span>
        <span className="text-emerald-600 dark:text-emerald-400">{open ? 'Hide' : 'Show'}</span>
      </button>
      {open && (
        <ol className="text-muted mt-3 list-decimal space-y-2 pl-5 text-sm leading-relaxed">
...
function RecipeCard({
  ...
  return (
    <li className="surface-card overflow-hidden p-4 shadow-lg">
      {recipe.imageUrl && (
        <img
          src={recipe.imageUrl}
          alt=""
      ...
      {analysis.have.length > 0 ? (
        <div className="mb-3">
          <p className="text-muted mb-1.5 text-xs font-semibold uppercase tracking-wide">
            What you have ({analysis.stockedCount})
          </p>
          <div className="flex flex-wrap gap-1.5">
            {analysis.have.map((ing) => (
              <span
                key={ing.name}
                className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-semibold text-emerald-900 ring-1 ring-emerald-300 dark:bg-emerald-950/70 dark:text-emerald-200 dark:ring-emerald-700"
              >
                {ing.name}
              </span>
            ))}
        ...
        <div className="mb-4">
          ...
          {showAddConfirm ? (
            ...
            <button
              ...
              className="w-full rounded-xl border-2 border-sky-200 bg-sky-50 py-2.5 text-sm font-bold text-sky-800 active:scale-[0.98] dark:border-sky-800 dark:bg-sky-950/40 dark:text-sky-200"
            >
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        <button
          ...
          className="rounded-xl border border-slate-200 bg-white py-2.5 text-xs font-bold text-slate-700 active:scale-[0.98] dark:border-slate-600 dark:bg-slate-900 dark:text-slate-200"
        >
          Mark as cooked
        </button>
        <button
          type="button"
          onClick={() => onToggleSave(recipe)}
          className="rounded-xl bg-violet-600 py-2.5 text-xs font-bold text-white active:scale-[0.98]"
        >
  ...
  const markCooked = (recipe) => {
    updateItems((prev) => {
      ...
      for (const ingredient of recipe.ingredients) {
        ...
        if (idx >= 0) {
          next[idx] = markItemOnShoppingList(next[idx]);
        }
      }
      return next;
    });
      ...
      <div className="mb-4 grid grid-cols-3 gap-2">
        {[
          ...
          return (
            ...
            >
              <Icon className={`h-4 w-4 ${id === RECIPE_VIEW.SAVED && active ? 'fill-current' : ''}`} />
              {label}
              {count > 0 && (
      ...
      {recipeView === RECIPE_VIEW.SEARCH && (
        <section className="surface-card mb-5 space-y-3 p-4">
          ...
          <p className="text-muted text-[11px] leading-relaxed">
            ...
            (free community recipe database). Built-in catalogue: {BUILTIN_RECIPES.length} household
            favourites.
          </p>
        </section>
      )}
